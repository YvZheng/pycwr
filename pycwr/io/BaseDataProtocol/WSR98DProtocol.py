...
class WSR98DFormat(object):
    ...
    def __init__(self):
        ...
        self.CutConfigurationBlockPos = 416

        self.CutConfigurationBlockSize = 256
        self.RadialHeaderBlockSize = 64
        ...
        TaskConfigurationBlock = (
            ...
            ('PulseWidth', INT),  # 脉冲宽度   units:Nanosecond
            ('VolumeStartTime', INT),  # 扫描开始时间 扫描开始时间为UTC标准时间计数,1970年1月1日0时为起始计数基准点 units:seconds
            ('CutNumber', INT),  # 扫描层数 根据扫描任务类型确定的扫描层数
            ('HorizontalNoise', FLOAT),  # 水平通道噪声      dBm
            ('VerticalNoise', FLOAT),  # 垂直通道噪声      dBm
            ('HorizontalCalibration', FLOAT),  # 水平通道标定值    dB
            ('VerticalCalibration', FLOAT),  # 垂直通道标定值    dB
            ('HorizontalNoiseTemperature', FLOAT),  # 水平通道噪声温度 K
            ('VerticalNoiseTemperature', FLOAT),  # 垂直通道噪声温度 K
            ('ZdrCalibration', FLOAT),  # ZDR标定偏差 dB
            ('PhaseCalibration', FLOAT),  # 差分相移标定偏差 degree
            ('LDRCalibration', FLOAT),  # 系统LDR标定偏差  dB
            ('Reserved03', '40s'),  # 保留字段
        ),
        CutConfigurationBlock = np.dtype([
            ...
            ('LogResolution', 'i4'),  # 强度分辨率 强度数据的距离分辨率 meter
            ('DopplerResolution', 'i4'),  # 多普勒数据的距离分辨率 meter
    ...
    def RadialHeader(self):
        RadialHeaderBlock = (
            ...
            ('Seconds', INT),  # 径向数据采集时间 UTC 从19700101 00：00开始 秒
